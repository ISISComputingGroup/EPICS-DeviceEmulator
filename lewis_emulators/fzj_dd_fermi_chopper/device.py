from collections import OrderedDict

from lewis.devices import StateMachineDevice
from .states import DefaultState


class SimulatedFZJDDFCH(StateMachineDevice):
    """
    Simulated FZJ Digital Drive Fermi Chopper Controller.
    """

    def _initialize_data(self):
        """
        Sets the initial state of the device.
        """
        self.chopper_name = "C01"
        # reference frequency set to 50Hz to match actual device
        self.frequency_reference = 50
        self.frequency_setpoint = 0
        self.frequency = 0
        self.phase_setpoint = 0
        self.phase = 0
        self.phase_status_is_ok = True
        self.magnetic_bearing_is_on = True
        self.magnetic_bearing_status_is_ok = True
        self.magnetic_bearing_integrator = 0
        self.drive_is_on = False
        self.drive_mode_is_start = False
        self.drive_l1_current = 0
        self.drive_l2_current = 0
        self.drive_l3_current = 0
        self.drive_direction_is_cw = True
        self.parked_open_status_is_ok = True
        self.drive_temperature = 0
        self.input_clock = 0
        self.phase_outage = 0
        self.master_chopper = "C01"
        self.logging_is_on = True
        self.lmsr_status_is_ok = True
        self.dsp_status_is_ok = True
        self.interlock_er_status_is_ok = True
        self.interlock_vacuum_status_is_ok = True
        self.interlock_frequency_monitoring_status_is_ok = True
        self.interlock_magnetic_bearing_amplifier_temperature_status_is_ok = True
        self.interlock_magnetic_bearing_amplifier_current_status_is_ok = True
        self.interlock_drive_amplifier_temperature_status_is_ok = True
        self.interlock_drive_amplifier_current_status_is_ok = True
        self.interlock_ups_status_is_ok = True

        self.error_on_set_frequency = None
        self.error_on_set_phase = None
        self.error_on_set_magnetic_bearing = None
        self.error_on_set_drive_mode = None

        self.disconnected = False

    def _get_state_handlers(self):
        """
        Returns: states and their names
        """
        return {DefaultState.NAME: DefaultState()}

    def _get_initial_state(self):
        """
        Returns: the name of the initial state
        """
        return DefaultState.NAME

    def _get_transition_handlers(self):
        """
        Returns: the state transitions
        """
        return OrderedDict()

    def reset(self):
        """
        Reset device to defaults
        :return: 
        """
        self._initialize_data()
