        ...
        while True:
            try:
                ...
                if data and "comp_p" in str(data):
                    if "?" in str(data):
                        client.send(("comp_p "+str(self.parent.comp_spin_P.value())+":").encode('utf-8'))
                    else:
                        self.comp_p.emit(float(re.findall(r"[-+]?\d*\.\d+|\d+", str(data))[0]))
                        client.send("comp_p:".encode('utf-8'))
                if data and "comp_a" in str(data):
                    if "?" in str(data):
                        client.send(("comp_a "+str(self.parent.comp_spin_A.value())+":").encode('utf-8'))
                    else:
                ...
                if data and "amp_p" in str(data):
                    #print("Amplitude")
                    if "?" in str(data):
                        client.send(("amp_p "+str(self.parent.amplitude_spin_P.value())+":").encode('utf-8'))
                ...
                if data and "amp_a" in str(data):
                    #print("Amplitude")
                    if "?" in str(data):
                        client.send(("amp_a "+str(self.parent.amplitude_spin_A.value())+":").encode('utf-8'))
                        #client.send("amp query received".encode('utf-8'))
                    else:
...
class Flippr(QtWidgets.QMainWindow, Ui_Flippr):
    ...
    def __init__(self):
        ...
        self.interrupted = 0
        self.running = 0  # Important state flag, 0 = both flippers off, 1 = P flipper on, 2 = A flipper on, 
        #3 = Both flippers on. This should ONLY be adjusted by the onoff() function
        self.running_previous = 0 # Flag to remember the previous state of running prior to turn off
        ...
        self.P_flipper_button.clicked[bool].connect(lambda pressed:self.toggle_button(pressed,flipper=1))

        self.A_flipper_Button.setChecked(False)
        self.A_flipper_Button.clicked[bool].connect(lambda pressed:self.toggle_button(pressed,flipper=2))
        ...
        def timeout():
            ...
            if (self.running == 1 or 2 or 3) and (np.abs(t - self.rtask.time) > 5):
                ...
                self.off()
                self.interrupted = 1
                print('Chopper/SMP signal interrupted, check signal cable (AFPI0) or beam status: FLIPPERS OFF')
    ...
    def init_tasks(self):
        ...
        self.atask_P = AnalogTask([self.decay_spin_P.value()],
                                    [self.amplitude_spin_P.value()],
                                    ['Dev1/ao1'],
                                    [self.DeltaT_P.value()],
                                    [self.filename_P])    # Analog signal output
    ...
    def on(self,flipperstate): #AJC 5/11/2018 altered all functions to be generic such that only at the global call of the function is the flipper decided
        ...
        if self.running != flipperstate:
            ...
            if flipperstate == 1:
                ...
                self.pulseOutput_P.plot_figure(np.arange(len(self.atask_P.write_list[0])), self.atask_P.write_list[0])
                ...
                self.pulseOutput_A.plot_figure(
                    np.arange(len(self.atask_A.write_list[0])), self.atask_A.write_list[0],format='b-')
                ...
                self.atask_AP = AnalogTask([self.decay_spin_P.value(),self.decay_spin_A.value()],
                ...
                self.pulseOutput_P.plot_figure(np.arange(len(self.atask_AP.write_list[0])), self.atask_AP.write_list[0])
                self.pulseOutput_A.plot_figure(np.arange(len(self.atask_AP.write_list[1])), self.atask_AP.write_list[1],format='b-')
                self.running_indicator.setText("RUNNING")
