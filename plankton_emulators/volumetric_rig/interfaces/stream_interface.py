...
class VolumetricRigStreamInterface(StreamAdapter):
    ...
    def get_gas_mix_matrix(self):
        ...
        lines = list()
        # Add column headers
        for i in range(max(column_headers, key=len)):
            words = list()
            # For the top-left block of white space
            words.append((max(row_titles, key=len)-1)*" ")
    ...
    def gas_mix_check(self, gas1_index, gas2_index):
        ...
        return ' '.join(["GMC",
                         gas1.index_string(), gas1.name(self.gas_output_length, '.'),
                         gas2.index_string(), gas2.name(self.gas_output_length, '.'),
    ...
    def _set_valve_status(self, buffer_number, set_to_open):
        if self.rig.halted:
    ...
    def set_valve_closed(self, buffer_number):
        self._set_valve_status(buffer_number, False)

    def set_valve_open(self, buffer_number):
        self._set_valve_status(buffer_number, True)
